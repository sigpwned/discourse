...
public class SubCommandDoesNotExtendSuperCommandScanException extends ScanException {
  private static final long serialVersionUID = -2114993557269612478L;

  private final Class<?> superclazz;
  private final Class<?> subclazz;

  public SubCommandDoesNotExtendSuperCommandScanException(Class<?> superclazz, Class<?> subclazz) {
    super(format("SubCommand class %s does not extend SuperCommand class %s", subclazz.getName(),
        superclazz.getName()));
    this.superclazz = requireNonNull(superclazz);
    this.subclazz = requireNonNull(subclazz);
    if (superclazz.isAssignableFrom(subclazz)) {
      // TODO Throwing an exception inside an exception constructor may not be a good idea...
      throw new IllegalArgumentException("subclazz must not extend superclazz");
    }
  }
