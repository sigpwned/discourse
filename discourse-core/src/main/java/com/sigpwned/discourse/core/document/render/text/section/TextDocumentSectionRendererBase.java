...
public abstract class TextDocumentSectionRendererBase implements DocumentSectionRenderer {
  @Override
  public boolean renderDocumentSection(Doc document, DocumentSection section, PrintStream out,
    ...
    Node text = getText(section).orElse(null);
    if (text == null)
      return false;
    ...
    try (PrintStream bufout = new PrintStream(buf, false, StandardCharsets.UTF_8)) {
      if (!nodeRenderer.renderNode(document, section, text, bufout, context))
        return false;
    }
    ...
    String wrappedText = Text.wrap(transformedText, width);

    out.println(wrappedText);
    out.println();

    return true;
  }

  protected abstract Optional<Node> getText(DocumentSection section);
